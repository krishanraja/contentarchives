...
| `stages/10_reclaim/dedupe_library.py` | delete the library's OWN duplicate copies, keeping one canonical file per group. `purge_redundant.py` and `already_in_library.py` both refuse anything inside the library by design - they delete outside copies the library makes redundant - so this is the opposite case and needs its own tool rather than a weakened version of theirs. The keeper rule is written down because a rule nobody can read is a coin toss with extra steps: curated `Media\Personal|Communal` first, `ContentProduction` working copies last, camera-native filenames beating app exports, then deterministic tiebreaks. Every removal goes through `guarded_delete`, and the inode check matters here more than anywhere - two names for one set of bytes free nothing (learning 28), so all 1,205 groups were checked first and none is a hardlink |
| `stages/10_reclaim/verify_archive_members.py` | is every media member of an ARCHIVE already held? The audit that cleared 1,094 GB only ever looked at files on disk, and a zip is one file to it - nothing inside was ever compared. `arc-progress/*.csv` records that each member was "handled", but that includes `dup` and `skipped`, and a progress log is a record of a DECISION, not evidence about content. Streams every member and hashes it without extracting: a 891 MB member read whole is what gets a process killed here |
