...
# E (face-emb.npy) and FACE-CLUSTERS.csv are paired by position; the check that
# they still line up lives in guards/ so every stage uses the same one.
from guards.alignment import check_alignment                     # noqa: E402,F401
...
def main() -> int:
    ...
    n = st.tag_many(out)
    print()
    print("wrote {:,} cluster tags. Name them with stages/07_people/answers.py:".format(n))
