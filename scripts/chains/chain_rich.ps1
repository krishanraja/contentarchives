...
for ($try = 1; $try -le 40; $try++) {
    ...
    Invoke-Step -Name "rich a$try" -ExpectedUnits ($left * 5.3) -CheckpointMin 15 `
        ...
        -Preflight {
            ...
            python -u "$repo\engine\classify_live.py" --thumbs D:\_thumbs `
            ...
            $chk = & python "$repo\tools\verify_rich.py" --sample 6 2>&1 | Out-String
        ...
        -Verify {
            ...
            $chk = & python "$repo\tools\verify_rich.py" --sample 30 2>&1 | Out-String
            foreach ($ln in ($chk -split "`n" | Where-Object { $_.Trim() })) { Say "    $ln" }
            return ($LASTEXITCODE -ne 1)
        } `
        -Postcondition {
...
python -u "$repo\tools\verify_rich.py" --sample 40 *>> $log
